...
console.log(
  parsedInput.reduce(
    (total, { range, letter, password }) =>
      total + isPasswordPositionValid(range, letter, password),
    0
  )
);
...
/**
 * @param {string} input
 * @returns {{ range: [number, number], letter: string, password: string }[]}
 */
function parseInput(input) {
  const lines = input.split("\n");
  return lines.map((line) => {
    const [, startRange, endRange, letter, password] = line
      .trim()
      .match(/^(\d+)-(\d+)\s(.):\s(.+)$/);

    return {
      range: [Number(startRange), Number(endRange)],
      letter,
      password,
    };
  });
 ...
 * @returns {number}
 */
function isPasswordValid(range, letter, password) {
  const occurrences = (password.match(new RegExp(letter, "g")) ?? []).length;
  const isValid = occurrences >= range[0] && occurrences <= range[1];
 ...
 * @returns {number}
 */
function isPasswordPositionValid(range, letter, password) {
  const isPos1 = password[range[0] - 1] === letter;
  const isPos2 = password[range[1] - 1] === letter;

  const isValid = !(isPos1 && isPos2 || !isPos1 && !isPos2);
